...
def v0Max(indice1, indice2):
    ...
    for i in range(0,len(s[0])):
        if i != 2:
            som += inversa[2][i] * (dados[indice1][i]-u[i])
    x1 -= som/(2*inversa[2][2])
    print('v0 estimado = ', x1, ' v0 certo', dados[indice1][2])
    x2 = u[2]
    som = 0
    for i in range(0,len(s[0])):
        if i != 2:
            som += inversa[2][i] * (dados[indice2][i]-u[i])
    x2 -= som/(2*inversa[2][2])
    print('v0 estimado = ', x2, ' v0 certo', dados[indice2][2])
